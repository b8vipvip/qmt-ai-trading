...
function statusTag(value: string) { const text = String(value || 'UNKNOWN'); const color = ['READY', 'SUCCESS', 'NORMAL', 'SAVED', 'EXISTS'].includes(text) ? 'green' : ['ERROR', 'FAILED', 'MISSING'].includes(text) ? 'red' : 'gold'; return <Tag color={color}>{text}</Tag>; }
...
const pathMeta: Record<QmtPathKey, { title: string; placeholder: string; scanText: string }> = {
  qmtClientPath: { title: 'QMT 客户端目录', placeholder: '例如 D:\\国金证券QMT交易端', scanText: '扫描QMT目录' },
...
export function SystemApiPage() {
  ...
  const [qmtBusy, setQmtBusy] = useState<QmtPathKey | 'all' | ''>('');
  ...
  const apiColumns = useMemo(() => [{title:'名称',dataIndex:'name',width:180},{title:'Provider',dataIndex:'provider',width:150,render:(v:string)=><Tag color="blue">{v}</Tag>},{title:'Base URL',dataIndex:'baseUrl',width:260},{title:'默认模型',dataIndex:'modelName',width:160},{title:'账号',dataIndex:'account',width:120},{title:'Token',dataIndex:'tokenMasked',width:120,render:(v:string,r:ApiConfigRow)=><Tag color={r.hasToken?'green':'default'}>{r.hasToken ? v : '未配置'}</Tag>},{title:'用途',dataIndex:'purposes',width:180,render:(v:string[])=>(v || []).length ? (v || []).map((x)=><Tag key={x}>{x}</Tag>) : <Tag>未分配</Tag>},{title:'启用',dataIndex:'enabled',width:90,render:(v:boolean)=><Tag color={v?'green':'default'}>{String(v)}</Tag>},{title:'更新时间',dataIndex:'updatedAt',width:180},{title:'来源',dataIndex:'sourcePath',width:120,render:(v:string)=><SourcePathTag value={v}/>},{title:'操作',width:180,render:(_:unknown,row:ApiConfigRow)=><Space><Button size="small" onClick={()=>openForm(row)}>编辑</Button><Button size="small" onClick={()=>test(row.id)}>测试</Button></Space>}], []);
  ...
  const saveQmt = async (kind?: QmtPathKey) => { const qmt = qmtForm.getFieldsValue(true); await saveSystemSettings({ ...settings, qmt }); message.success(kind ? `${pathMeta[kind].title} 已保存` : 'QMT 路径已保存'); window.dispatchEvent(new Event('qmt-settings-saved')); };
  const scanQmt = async (kind: QmtPathKey) => { try { setQmtBusy(kind); const result = await scanQmtPaths(kind); setQmtCandidates((prev)=>[...result.candidates, ...prev].filter((x,i,arr)=>arr.findIndex((y)=>y.kind===x.kind && y.path===x.path)===i)); const first = (result.candidates || []).find((x)=>x.kind===kind && x.exists) || (result.candidates || []).find((x)=>x.kind===kind); if (first) { qmtForm.setFieldsValue({ [kind]: first.path, clientName: first.clientName } as any); } message.success(`扫描完成，发现 ${result.count} 个候选路径`); } catch (error) { message.error(error instanceof Error ? error.message : String(error)); } finally { setQmtBusy(''); } };
  const testQmt = async (kind: QmtPathKey) => { try { setQmtBusy(kind); await saveQmt(kind); const res = await testQmtSettings(kind); setQmtTestResults((prev)=>({ ...prev, [kind]: res })); if (res.status === 'READY') message.success(`${pathMeta[kind].title} 测试通过`); else message.warning(res.message); } catch (error) { message.error(error instanceof Error ? error.message : String(error)); } finally { setQmtBusy(''); } };
  ...
  const pathField = (kind: QmtPathKey) => <Row gutter={12} align="bottom"><Col xs={24} md={18}><Form.Item name={kind} label={pathMeta[kind].title}><AutoComplete options={optionsFor(kind)} onSelect={(value)=>{ const c = candidateByPath(kind, value); if (c) qmtForm.setFieldsValue({ clientName: c.clientName }); }}><Input.Search enterButton="选择/扫描" onSearch={()=>scanQmt(kind)} placeholder={pathMeta[kind].placeholder} /></AutoComplete></Form.Item></Col><Col xs={24} md={6}><Space wrap><Button loading={qmtBusy===kind} onClick={()=>scanQmt(kind)}>{pathMeta[kind].scanText}</Button><Button onClick={()=>saveQmt(kind)}>保存</Button></Space></Col></Row>;
  const qmtRows = ([['qmtClientPath','QMT客户端目录'], ['xtdataPath','xtdata数据目录'], ['xtquantPythonPath','xtquant Python目录']] as [QmtPathKey,string][]).map(([kind,label])=>({ kind, label, path: (settings.qmt as any)[kind] || '', clientName: settings.qmt.clientName || '-', status: qmtTestResults[kind]?.status || ((settings.qmt as any)[kind] ? '已保存' : '未配置'), message: qmtTestResults[kind]?.message || '' }));

  return <div className="page-grid">
    <Section title="QMT / xtdata 本地路径">
      <Form form={qmtForm} layout="vertical"><Form.Item name="clientName" hidden><Input /></Form.Item>{pathField('qmtClientPath')}{pathField('xtdataPath')}{pathField('xtquantPythonPath')}</Form>
      <Table rowKey="kind" size="small" dataSource={qmtRows} pagination={false} columns={[{title:'类型',dataIndex:'label',width:180},{title:'识别客户端',dataIndex:'clientName',width:160},{title:'已配置路径',dataIndex:'path',render:(v)=><Typography.Text copyable={!!v}>{v || '未配置'}</Typography.Text>},{title:'状态',dataIndex:'status',width:120,render:statusTag},{title:'测试信息',dataIndex:'message',ellipsis:true},{title:'操作',width:180,render:(_,row)=><Space><Button size="small" onClick={()=>saveQmt(row.kind)}>保存</Button><Button size="small" loading={qmtBusy===row.kind} onClick={()=>testQmt(row.kind)}>测试连接</Button></Space>}]} scroll={{ x: 1000 }} />
    </Section>
    <Section title="外部 API / AI 服务接口配置" extra={<Button type="primary" onClick={() => openForm()}>新增 API</Button>}><Table rowKey="id" size="small" dataSource={apiConfigs} columns={apiColumns as any} scroll={{ x: 1750, y: 430 }} locale={{ emptyText: <EmptyState text="暂无 API；点击“新增 API”配置 AI 接口、AkShare、Tushare、BaoStock 或自定义 HTTP。" /> }} /></Section>
