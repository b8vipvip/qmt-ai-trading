import { Button, Card, Col, Descriptions, Divider, Form, Input, InputNumber, message, Modal, Row, Segmented, Space, Table, Tabs, Tag, Typography } from 'antd';
...
function TaskButton({ taskId, params, children, size = 'small', type, danger }: { taskId: string; params?: Record<string, unknown>; children: ReactNode; size?: 'small' | 'middle' | 'large'; type?: 'primary' | 'default' | 'dashed' | 'link' | 'text'; danger?: boolean }) {
  ...
  const run = async () => {
    try {
      ...
      if (task?.status === 'SUCCESS') {
        message.success(`${task.task_name || taskId} 执行完成`);
      } else {
        message.warning(`${task?.task_name || taskId} 状态：${task?.status || 'UNKNOWN'}`);
      }
  ...
  return <Button size={size} type={type} danger={danger} loading={running} onClick={run}>{children}</Button>;
}
...
export function DashboardPage() {
  ...
  return <div className="page-grid">
    <Section title="工作流快捷启动" extra={<Tag color="green">所有任务走 /api/v1/tasks/run</Tag>}><Space wrap><TaskButton taskId="xtdata_live_readonly_smoke" type="primary">刷新只读行情</TaskButton><TaskButton taskId="factor_scan">多因子扫描</TaskButton><TaskButton taskId="strategy_dry_run_signals" params={{ symbol: '510300.SH' }}>策略信号</TaskButton><TaskButton taskId="risk_gate_dry_run">风控检查</TaskButton><TaskButton taskId="paper_trading_dry_run">Paper Trading</TaskButton><TaskButton taskId="human_approval_review_dry_run">人工复核</TaskButton><TaskButton taskId="order_preview_dry_run">订单预览</TaskButton></Space></Section>
    <Row gutter={[16, 16]}>{overview.metrics.map((m) => <Col xs={24} sm={12} lg={6} xl={6} key={m.title}><MetricCard item={m} /></Col>)}</Row>
    <Row gutter={[16, 16]}><Col xs={24} xl={16}><Section title="账户资产曲线" extra={<Segmented options={['今日','近7日','近30日','近90日','今年','全部']} defaultValue="近90日" />}><EquityCurveChart data={curve} /></Section></Col><Col xs={24} xl={8}><Section title="实时风险面板" extra={<SourcePathTag value={risk.sourcePath} />}><Row gutter={[12, 12]}><Col span={12}><RiskGauge title="单票集中度" value={risk.concentration ?? 0} /></Col><Col span={12}><RiskGauge title="行业集中度" value={risk.industry ?? 0} /></Col><Col span={12}><RiskGauge title="总仓位" value={risk.total ?? risk.totalPosition ?? 0} /></Col><Col span={12}><RiskGauge title="当日亏损" value={risk.dayLoss ?? 0} /></Col></Row></Section></Col></Row>
    <Section title="策略运行状态" extra={<TaskButton taskId="strategy_dry_run_signals" params={{ symbol: '510300.SH' }}>重新生成信号</TaskButton>}><StrategyStatusTable data={strategies} onDanger={setDanger} /></Section><Section title="系统事件流"><EventLogPanel events={events} /></Section><ConfirmDangerActionModal open={!!danger} title={danger} confirmText="确认停止交易" onCancel={() => setDanger('')} onConfirm={() => setDanger('')} />
  </div>;
}
...
export function DataCenterPage() {
  ...
  return <div className="page-grid">
    <Section title="数据任务快捷启动"><Space wrap><TaskButton taskId="xtdata_live_readonly_smoke" type="primary">真实 xtdata 只读 smoke</TaskButton><TaskButton taskId="qmt_data_diagnostics_readonly">QMT 行情只读诊断</TaskButton><TaskButton taskId="market_snapshot_readonly">只读行情快照</TaskButton><TaskButton taskId="data_cache_check">本地缓存质量检查</TaskButton></Space></Section>
...
export function FactorsPage() {
  ...
  return <div className="page-grid">
    <Section title="因子任务快捷启动"><Space wrap><TaskButton taskId="factor_scan" type="primary">多因子扫描</TaskButton><TaskButton taskId="factor_research_dry_run">因子研究</TaskButton><TaskButton taskId="research_score_etf">ETF 研究评分</TaskButton><TaskButton taskId="model_lab_evaluate">模型实验评估</TaskButton></Space></Section>
    <Section title="因子库 / 后端候选池"><Table rowKey="id" size="small" dataSource={factors} columns={[{ title:'因子名称', dataIndex:'name' },{ title:'类型', dataIndex:'type' },{ title:'版本', dataIndex:'version' },{ title:'股票池/标的', dataIndex:'universe' },{ title:'评分', dataIndex:'score' },{ title:'排名', dataIndex:'rank' },{ title:'IC均值', dataIndex:'icMean' },{ title:'Rank IC', dataIndex:'rankIc' },{ title:'ICIR', dataIndex:'icir' },{ title:'原因', dataIndex:'reasons' },{ title:'风险提示', dataIndex:'riskFlags' },{ title:'状态', dataIndex:'status', render:(v)=> <Tag color={v==='已上线'?'green':v==='已下线'?'default':'blue'}>{v}</Tag> },{ title:'来源', dataIndex:'sourcePath', render:(v)=> <SourcePathTag value={v} /> },{ title:'操作', render:()=> <Space><TaskButton taskId="factor_research_dry_run">查看分析</TaskButton><TaskButton taskId="factor_strategy_dry_run">加入组合</TaskButton><Button size="small">下线</Button></Space> }]} scroll={{ x: 1700 }} locale={{ emptyText: <EmptyState text="暂无因子候选；请运行 factor_scan。" /> }} /></Section>
...
export function StrategiesPage() {
  ...
  const [danger, setDanger] = useState('');
  return <div className="page-grid">
    <Section title="策略任务快捷启动"><Space wrap><TaskButton taskId="strategy_dry_run_signals" type="primary" params={{ symbol: '510300.SH' }}>策略 dry-run 信号</TaskButton><TaskButton taskId="factor_strategy_dry_run">因子策略联调</TaskButton><TaskButton taskId="etf_rotation_candidates">ETF 轮动候选</TaskButton><TaskButton taskId="daily_pipeline_dry_run">日线流水线</TaskButton></Space></Section>
    <Section title="策略列表 / Strategy artifacts"><StrategyStatusTable data={strategies} onDanger={setDanger} /></Section>
    {detail && <Section title="策略详情"><Tabs items={[{key:'base',label:'基本信息',children:<Descriptions column={3} bordered size="small" items={[{key:'1',label:'策略',children:detail.strategy.name},{key:'2',label:'股票池',children:detail.strategy.pool},{key:'3',label:'调仓周期',children:detail.strategy.rebalance},{key:'4',label:'最新信号',children:detail.strategy.lastAction || '-'},{key:'5',label:'来源',children:<SourcePathTag value={detail.strategy.sourcePath} />}]} />},{key:'signal',label:'信号规则',children:<Space wrap><Typography.Text>当前接入 strategy_signals / trade_intents 后端产物。</Typography.Text><TaskButton taskId="strategy_dry_run_signals" params={{ symbol: detail.strategy.pool || '510300.SH' }}>重新生成</TaskButton></Space>},{key:'portfolio',label:'组合构建',children:<Table size="small" rowKey="code" dataSource={detail.holdings} columns={[{title:'股票',dataIndex:'name'},{title:'当前权重',dataIndex:'currentWeight'},{title:'目标权重',dataIndex:'targetWeight'},{title:'来源',dataIndex:'sourcePath',render:(v)=> <SourcePathTag value={v} />}]} locale={{ emptyText: <EmptyState /> }} />},{key:'risk',label:'风控配置',children:<TaskButton taskId="risk_gate_dry_run">运行 Risk Gate</TaskButton>},{key:'backtest',label:'回测结果',children:<Space direction="vertical"><TaskButton taskId="backtest_report">生成回测报告</TaskButton><DrawdownChart data={[]} /></Space>},{key:'live',label:'实盘表现',children:'当前处于 dry-run / shadow / read-only 阶段，不允许真实发单。'},{key:'logs',label:'日志',children:<EventLogPanel events={[]} />}]} /></Section>}
    {plan && <Section title="调仓计划 / 只进风控不下单" extra={<TaskButton taskId="order_preview_dry_run">生成订单预览</TaskButton>}><Table rowKey="code" size="small" dataSource={plan.holdings} columns={[{title:'股票代码',dataIndex:'code'},{title:'股票名称',dataIndex:'name'},{title:'当前持仓',dataIndex:'currentQty'},{title:'当前权重',dataIndex:'currentWeight'},{title:'目标权重',dataIndex:'targetWeight'},{title:'建议买卖数量',dataIndex:'diffQty'},{title:'差异金额',dataIndex:'diffAmount',render:money},{title:'风控',dataIndex:'riskStatus',render:(v)=> <RiskLevelBadge level={v} />},{title:'来源',dataIndex:'sourcePath',render:(v)=> <SourcePathTag value={v} />}]} scroll={{ x: 1200, y: 260 }} locale={{ emptyText: <EmptyState /> }} /><Typography.Text type="warning">{plan.notice}</Typography.Text></Section>}
...
export function DeploymentPage() {
  ...
  const [danger, setDanger] = useState('');
  const stageTask = (stage: string) => stage.includes('Paper') ? 'paper_trading_dry_run' : stage.includes('Shadow') ? 'shadow_replay_backtest' : stage.includes('Small') ? 'live_readiness_blockers_review' : 'live_readiness_blockers_review';
  return <div className="page-grid deployment-board">{stages.map((s) => <Card key={s.stage} className={`stage-card ${s.stage.includes('Full') ? 'critical' : ''}`} title={s.stage} extra={<Tag color={String(s.status).includes('LOCKED')?'red':'blue'}>{s.status}</Tag>}><Descriptions column={1} size="small" items={[{key:'1',label:'开始时间',children:s.startedAt},{key:'2',label:'运行天数',children:s.days},{key:'3',label:'策略数量',children:s.strategyCount},{key:'4',label:'资金规模',children:s.capital},{key:'5',label:'当前问题',children:s.issues},{key:'6',label:'来源',children:<SourcePathTag value={s.sourcePath} />}]} /><Divider /><b>通过标准</b><ul>{(s.criteria || []).map((c:string) => <li key={c}>{c}</li>)}</ul><Space wrap><TaskButton taskId={stageTask(s.stage)}>运行阶段检查</TaskButton><Button danger={s.stage.includes('Full')} onClick={() => setDanger(`阶段推进：${s.stage}`)}>{s.stage.includes('Full') ? '申请进入正式实盘' : '提交阶段复核'}</Button></Space></Card>)}<ConfirmDangerActionModal open={!!danger} title={danger} confirmText="确认风险已知" onCancel={() => setDanger('')} onConfirm={() => setDanger('')} /></div>;
}
...
export function ExecutionPage() {
  ...
  const [danger, setDanger] = useState('');
  return <div className="page-grid">
    <Section title="交易执行任务快捷启动"><Space wrap><TaskButton taskId="account_readonly_dry_run" type="primary" params={{ enable_account_readonly: true, manual_confirmed: true, allow_import_xttrader: true, allow_connect_trade_session: true, allow_account_query: true, allow_position_query: true }}>账户/持仓只读</TaskButton><TaskButton taskId="order_preview_dry_run">生成订单预览</TaskButton><TaskButton taskId="paper_trading_dry_run">Paper Trading</TaskButton><TaskButton taskId="human_approval_review_dry_run">人工审批复核</TaskButton></Space></Section>
...
export function RiskCenterPage() {
  ...
  const [danger, setDanger] = useState('');
  return <div className="page-grid">
    <Section title="风控任务快捷启动"><Space wrap><TaskButton taskId="risk_gate_dry_run" type="primary">Risk Gate dry-run</TaskButton><TaskButton taskId="human_approval_review_dry_run">人工复核</TaskButton><TaskButton taskId="monitoring_alert_dry_run">监控告警</TaskButton><TaskButton taskId="live_readiness_blockers_review">实盘阻断项复核</TaskButton></Space></Section>
...
export function MonitoringPage() {
  ...
  return <div className="page-grid">
    <Section title="监控任务快捷启动"><Space wrap><TaskButton taskId="monitoring_alert_dry_run" type="primary">生成监控告警</TaskButton><TaskButton taskId="generate_daily_report">生成日报</TaskButton><TaskButton taskId="list_latest_reports">刷新报告列表</TaskButton></Space></Section>
...
export function SettingsPage() {
  ...
  return <div className="page-grid">
    <Section title="系统任务快捷启动"><Space wrap><TaskButton taskId="list_latest_reports" type="primary">最新报告列表</TaskButton><TaskButton taskId="ai_model_discovery">AI 模型发现</TaskButton><TaskButton taskId="ai_model_usage_draft">模型用途映射草稿</TaskButton></Space></Section>
