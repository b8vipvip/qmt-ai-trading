...
function DownloadedDataList() {
  ...
  return <Section title="当前已下载的数据列表" extra={<Tag color="green">已就绪 {data.readyCount}/{data.totalCount}</Tag>}>
    <Table rowKey="key" size="small" dataSource={data.rows || []} columns={columns} scroll={{ x: 1180, y: 320 }} locale={{ emptyText: <EmptyState text="暂无已下载数据；先同步 QMT 真实列表或开启自动刷新行情。" /> }} />
  </Section>;
}
