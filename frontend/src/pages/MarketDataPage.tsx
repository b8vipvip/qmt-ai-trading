...
import { getMarketAutoRefreshStatus, getMarketQuotes, getMarketSummary, getMarketUniverseStatus, getQmtUniverseSyncStatus, runMarketAIAnalysis, runQmtUniverseSync, saveMarketAutoRefresh, saveMarketUniverse } from '../services/marketDataService';
import type { MarketAIAnalysis, MarketAutoRefreshStatus, MarketQuoteRow, MarketSummary, MarketUniverseStatus, QmtUniverseGroup, QmtUniverseSyncResult } from '../services/marketDataService';
...
function statusTag(value: string) { const text = String(value || 'UNKNOWN'); return <Tag color={text === 'NORMAL' || text === 'READY' || text === 'REAL_MARKET_DATA' ? 'green' : text === 'PENDING' || text === 'WAITING_LOGIN_OR_FALLBACK' || text === 'NOT_SYNCED' ? 'gold' : text === 'FAILED' ? 'red' : 'default'}>{text}</Tag>; }
...
function QmtUniverseSyncControl({ onChanged }: { onChanged: () => void }) {
  ...
  useEffect(() => { load(); }, []);
  const run = async () => {
    try { setRunning(true); const res = await runQmtUniverseSync(['all_a', 'etf', 'index']); setSync(res); message.success(res.message || 'QMT 真实列表同步完成'); onChanged(); }
  ...
  const apply = async (group: QmtUniverseGroup) => {
    try { setApplyingKey(group.key); const next = await saveMarketUniverse({ preset: 'custom', customSymbolsText: (group.symbols || []).join('\n') }); message.success(`已应用到 Universe：${group.name} → ${next.symbolCount} 个标的`); window.dispatchEvent(new Event('qmt-market-universe-updated')); window.dispatchEvent(new Event('qmt-market-auto-refresh-updated')); onChanged(); }
    catch (error) { message.error(error instanceof Error ? error.message : String(error)); }
    finally { setApplyingKey(''); }
  };
  const columns: ColumnsType<QmtUniverseGroup> = [
    { title: '列表', dataIndex: 'name', width: 180 },
    { title: '状态', dataIndex: 'status', width: 90, render: statusTag },
    { title: '标的数', dataIndex: 'symbolCount', width: 90 },
    { title: '同步时间', dataIndex: 'syncedAt', width: 170, render: formatMarketTime },
    { title: 'sector 调用', dataIndex: 'sectorResults', render: (items?: QmtUniverseGroup['sectorResults']) => <Space wrap>{(items || []).map((x) => <Tag key={x.sector} color={x.status === 'READY' ? 'green' : x.status === 'EMPTY' ? 'gold' : 'red'}>{x.sector}:{x.count}</Tag>)}</Space> },
    { title: '操作', width: 170, render: (_, row) => <Space><Button size="small" disabled={!row.symbolCount} loading={applyingKey === row.key} onClick={() => apply(row)}>应用到Universe</Button></Space> },
  ];
  return <Space direction="vertical" style={{ width: '100%' }} size={12}>
    ...
    <Table rowKey="key" size="small" dataSource={sync.groups || []} columns={columns} pagination={false} scroll={{ x: 900 }} locale={{ emptyText: <EmptyState text="尚未同步 QMT 真实股票列表；点击上方按钮后会调用 xtdata.get_stock_list_in_sector。" /> }} />
  </Space>;
}
...
function UniverseControl({ onChanged }: { onChanged: () => void }) {
  ...
  const load = async () => { try { const next = await getMarketUniverseStatus(); setState(next); setPreset(next.preset || 'broad_etf'); setCustomSymbolsText(next.preset === 'custom' ? (next.customSymbolsText || next.symbols.join('\n')) : (next.customSymbolsText || '')); } catch { setState(universeFallback); } };
  ...
  const save = async () => {
    try { setSaving(true); const next = await saveMarketUniverse({ preset, customSymbolsText }); setState(next); setPreset(next.preset); setCustomSymbolsText(next.preset === 'custom' ? (next.customSymbolsText || next.symbols.join('\n')) : (next.customSymbolsText || '')); message.success(`行情 Universe 已保存：${next.name}，${next.symbolCount} 个标的`); window.dispatchEvent(new Event('qmt-market-universe-updated')); window.dispatchEvent(new Event('qmt-market-auto-refresh-updated')); onChanged(); }
    catch (error) { message.error(error instanceof Error ? error.message : String(error)); }
  ...
  return <Space direction="vertical" style={{ width: '100%' }} size={12}>
    <Row gutter={[12, 12]} align="top"><Col xs={24} lg={8}><Space direction="vertical" style={{ width: '100%' }}><Typography.Text>Universe 类型</Typography.Text><Select value={preset} onChange={setPreset} style={{ width: '100%' }} options={[...state.presets.map((p) => ({ value: p.key, label: `${p.name}（${p.symbolCount}）` })), { value: 'custom', label: '自定义股票池' }]} /></Space></Col><Col xs={24} lg={16}><Space direction="vertical" style={{ width: '100%' }}><Typography.Text>自定义标的代码</Typography.Text><TextArea rows={3} disabled={preset !== 'custom'} value={preset === 'custom' ? customSymbolsText : preview.join('\n')} onChange={(e) => setCustomSymbolsText(e.target.value)} placeholder="每行一个或用逗号分隔，例如：510300.SH、600519.SH、000001.SZ" /></Space></Col></Row>
    <Space wrap><Button size="small" type="primary" loading={saving} onClick={save}>保存 Universe</Button><Tag color="blue">当前：{state.name}</Tag><Tag color="cyan">标的数：{state.symbolCount}</Tag><Tag color="purple">来源：{state.source === 'custom' ? '自定义' : '预设'}</Tag><span className="muted">自动刷新行情会按当前 Universe 只下载本地缺失缓存。</span></Space>
...
function AutoRefreshControl({ onChanged }: { onChanged: () => void }) {
  ...
  return <Space direction="vertical" style={{ width: '100%' }} size={12}><Space wrap size={16}><Space><span>自动刷新行情</span><Switch checked={status.enabled} loading={saving} onChange={(checked) => save({ enabled: checked, intervalSec })} /></Space><Space><span>间隔</span><InputNumber min={10} max={3600} value={intervalSec} onChange={(value) => setIntervalSec(Number(value) || 60)} disabled={saving} /><span>秒</span><Button size="small" loading={saving} onClick={() => save({ intervalSec })}>保存</Button></Space><Space><span>只下载缺失缓存</span><Switch checked={status.onlyMissingCache} loading={saving} onChange={(checked) => save({ onlyMissingCache: checked })} /></Space><Tag color="blue">Universe：{status.universeName || '-'}</Tag><Tag color="cyan">标的：{status.universeSymbolCount || 0}</Tag>{statusTag(status.lastStatus)}{status.threadAlive && <Tag color="blue">后台循环中</Tag>}{status.running && <Tag color="cyan">正在检查/刷新</Tag>}</Space><Alert type={status.lastRealMarketData ? 'success' : status.enabled ? 'warning' : 'info'} showIcon message={status.lastMessage || '自动刷新未启动'} description={`最近运行：${formatMarketTime(status.lastRunAt) || '-'}；运行次数：${status.runCount || 0}；真实行情：${status.lastRealMarketData ? '是' : '否'}；沙盒回退：${status.lastSandboxFallback ? '是' : '否'}`} /><Typography.Text type="secondary">开启后，后端会循环检查 QMT 客户端和 xtdata 登录状态；客户端未启动会尝试启动，未登录会提示并持续检测。连接成功后按当前 Universe 增量刷新本地缺失行情缓存。全程只读，不查询账户、不连接 xttrader、不下单。</Typography.Text></Space>;
}
...
export function MarketDataPage() {
  ...
  return <div className="page-grid"><Section title={<Space size={10}><span>行情数据用途</span><Button size="small" type="primary" loading={analyzing} onClick={analyze}>AI 分析行情</Button></Space>}><Typography.Paragraph>行情数据是量化系统的“价格与成交事实层”，用于生成 K 线、计算收益率和波动率、更新因子、触发策略信号、估算成交成本、校验停牌/涨跌停/流动性，并为回测、仿真、影子实盘和风控提供统一输入。</Typography.Paragraph><Space wrap><Tag color="cyan">采集：QMT xtdata / 本地缓存 / 外部数据源</Tag><Tag color="green">清洗：复权 / 对齐 / 去异常 / 去重复</Tag><Tag color="gold">校验：延迟 / 缺失 / 异常 / 覆盖率</Tag><Tag color="purple">服务：因子研究 / 策略信号 / 风控 / 监控复盘</Tag></Space></Section><Section title="行情 Universe 配置" extra={<Tag color="blue">决定自动刷新下载哪些标的</Tag>}><UniverseControl onChanged={() => setRefreshTick((x) => x + 1)} /></Section><Section title="自动刷新行情" extra={<Tag color="green">只读 / dry-run / 不触发交易</Tag>}><AutoRefreshControl onChanged={() => setRefreshTick((x) => x + 1)} /></Section><Row gutter={[16, 16]}><Col xs={24} md={8}><Card className="metric-card"><b>行情记录数</b><div className="metric-value">{summary.quoteCount}</div><span>当前快照记录数量</span></Card></Col><Col xs={24} md={8}><Card className="metric-card"><b>Universe 标的数</b><div className="metric-value">{summary.universeSymbolCount || summary.symbolCount}</div><span>{summary.universeName || '当前行情池'}；缓存标的 {summary.symbolCount}</span></Card></Col><Col xs={24} md={8}><Card className="metric-card"><b>最新时间</b><div className="metric-value" style={{ fontSize: 18 }}>{latestTime}</div><Space wrap>{dataModeTag(summary)}<span>{summary.sandboxFallback ? '当前为沙盒样例时间，不是真实市场时间' : '用于判断行情是否滞后'}</span></Space></Card></Col></Row><Section title="AI 行情分析报告"><AnalysisReport report={analysis} error={analysisError} /></Section><Section title="数据源状态"><Row gutter={[16, 16]}>{sources.map((s) => <Col xs={24} sm={12} lg={6} key={`${s.name}-${s.qualityLevel || ''}`}><Card className="source-card"><Space direction="vertical"><Space><b>{s.name}</b>{statusTag(String(s.status))}</Space><span>更新时间：{formatMarketTime(s.updatedAt || '-')}</span><span>今日记录：{Number(s.records || 0).toLocaleString()}</span><span>延迟：{s.latency}</span><span>质量：{s.qualityLevel || '-'}</span><span>缺失/异常：{s.missingRate}% / {s.abnormalRate}%</span></Space></Card></Col>)}</Row></Section><Section title="行情明细表"><Table rowKey={(record, index) => `${record.symbol}-${record.time}-${index ?? 0}`} size="small" dataSource={quotes} columns={columns} scroll={{ x: 1380, y: 420 }} locale={{ emptyText: <EmptyState text="暂无行情明细；开启自动刷新行情后，系统会按 Universe 从本地 QMT / xtdata 获取并缓存。" /> }} /></Section><Section title="行情数据质量"><Table rowKey={(record, index) => `${record.dataset}-${record.tradeDate}-${index ?? 0}`} size="small" dataSource={quality} columns={[{ title:'数据集', dataIndex:'dataset' },{ title:'交易日', dataIndex:'tradeDate', render: formatMarketTime },{ title:'覆盖股票数', dataIndex:'stockCount' },{ title:'缺失字段数', dataIndex:'missingFields' },{ title:'异常值数量', dataIndex:'abnormalValues' },{ title:'重复记录数', dataIndex:'duplicateRows' },{ title:'校验', dataIndex:'passed', render:(v)=> <Tag color={v?'green':'red'}>{v?'通过':'失败'}</Tag> }]} scroll={{ x: 950 }} locale={{ emptyText: <EmptyState text="暂无行情质量产物；自动刷新行情或数据质量任务运行后自动生成。" /> }} /></Section></div>;
}
