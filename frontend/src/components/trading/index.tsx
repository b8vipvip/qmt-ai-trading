import { Button, Space, Table, Tag, Timeline, Tooltip } from 'antd';
...
export function StrategyStatusTable({ data, onDanger }: { data: StrategyStatus[]; onDanger?: (title: string) => void }) {
  const columns: ColumnsType<StrategyStatus> = [
    ...
    { title: '最新信号', dataIndex: 'lastAction', width: 130, render: (v) => v ? <Tag color="cyan">{v}</Tag> : '-' },
    ...
    { title: '仓位/目标权重', dataIndex: 'position', width: 120, render: (v) => `${v}%` },
    ...
    { title: '产物路径', dataIndex: 'sourcePath', width: 180, render: (v) => v ? <Tooltip title={v}><span className="muted">artifact</span></Tooltip> : '-' },
  ...
  return <Table rowKey="id" size="small" columns={columns} dataSource={data} scroll={{ x: 1600, y: 360 }} pagination={false} />;
}
...
export function RiskRuleTable({ data, onDanger }: { data: RiskRule[]; onDanger: (title: string) => void }) {
  const columns: ColumnsType<RiskRule> = [
    ...
    { title: '说明', dataIndex: 'description', width: 180, render: (v) => v || '-' },
  ...
  return <Table rowKey="id" size="small" columns={columns} dataSource={data} scroll={{ x: 1180, y: 390 }} pagination={false} />;
}
...
export function BacktestReportPanel({ task }: { task: BacktestTask }) {
  return <div className="report-summary"><Tag color="blue">{task.name}</Tag><Tag>股票池：{task.universe}</Tag><Tag>区间：{task.range}</Tag><Tag color="green">状态：{task.status}</Tag>{task.sourcePath && <Tag color="purple">backend artifact</Tag>}<p>策略评价摘要：收益曲线平滑，最大回撤可控，超额收益主要来自因子暴露与行业轮动；需要继续验证小盘流动性与冲击成本。</p></div>;
}
