...
export function StrategyStatusTable({ data, onDanger }: { data: StrategyStatus[]; onDanger?: (title: string) => void }) {
  const columns: ColumnsType<StrategyStatus> = [
    { title: '策略名称', dataIndex: 'name', fixed: 'left', width: 210 },
    ...
    { title: '仓位', dataIndex: 'position', width: 90, render: (v) => `${v}%` },
    { title: '信号', dataIndex: 'signalCount', width: 80 },
    ...
    { title: '操作', width: 180, render: (_, row) => <Space><Button size="small">查看</Button><Button size="small">暂停</Button><Button danger size="small" onClick={() => onDanger?.(`停止策略：${row.name}`)}>停止</Button></Space> },
  ];
  return <Table rowKey="id" size="small" columns={columns} dataSource={data} scroll={{ x: 1180, y: 360 }} pagination={false} />;
}

export function RiskRuleTable({ data, onDanger }: { data: RiskRule[]; onDanger: (title: string) => void }) {
  const columns: ColumnsType<RiskRule> = [
    { title: '规则名称', dataIndex: 'name', width: 180 }, { title: '类型', dataIndex: 'type', width: 90 }, { title: '阈值', dataIndex: 'threshold', width: 110 }, { title: '当前值', dataIndex: 'current', width: 110 },
    { title: '动作', dataIndex: 'action', width: 110, render: (v) => <Tag color={['停止交易', '平仓', '拦截'].includes(v) ? 'red' : 'gold'}>{v}</Tag> },
    { title: '启用', dataIndex: 'enabled', width: 80, render: (v) => <Tag color={v ? 'green' : 'default'}>{String(v)}</Tag> }, { title: '最近触发', dataIndex: 'lastTriggered', width: 150 },
  ...
  return <Table rowKey="id" size="small" columns={columns} dataSource={data} scroll={{ x: 1000, y: 390 }} pagination={false} />;
}
...
export function OrderStatusTimeline({ status }: { status: string }) {
  ...
  return <Timeline items={states.slice(0, Math.max(1, idx + 1)).map((s) => ({ color: s === status ? 'blue' : 'green', children: s }))} />;
}

export function BacktestReportPanel({ task }: { task: BacktestTask }) {
  return <div className="report-summary"><Tag color="blue">{task.name}</Tag><Tag>股票池：{task.universe}</Tag><Tag>区间：{task.range}</Tag><Tag color="green">状态：{task.status}</Tag><p>策略评价摘要：收益曲线平滑，最大回撤可控，超额收益主要来自因子暴露与行业轮动；需要继续验证小盘流动性与冲击成本。</p></div>;
}
