...
def format_live_consistency_report_markdown(report:LiveConsistencyReport)->str:
    lines=["# Stage49 Remediation Read-only Review and Material Consistency Check","","## Decision",str(enum_value(report.decision)),"","## Safety Note",report.safety_note,"READY_FOR_CONSISTENCY_REVIEW 只表示补证后只读复核与一致性材料可供人工复核，不是实盘授权。","","## Evidence Summary",f"- total_evidence: {report.summary.get('total_evidence',len(report.evidence))}",f"- critical: {report.summary.get('critical',0)}",f"- warnings: {report.summary.get('warnings',len(report.warnings))}","- read_only: True","- dry_run_only: True","- no_trade_authorization: True","- live_trading_enabled: False","- no_task_registered: True"]
