...
def _warn_context(path='', generated=False):
    ...
    return generated or p.startswith('docs/') or '/tests/' in p or 'test_' in p or any(x in p for x in ['stage55','stage56','stage57','stage58','stage59','stage60','stage61','stage62','stage63','stage64','stage65','stage66','local_console_binding'])
def classify_local_console_binding_marker(marker, path='', generated=False, executable=False):
...
def scan_local_console_binding_asset_for_forbidden_markers(text, path='', generated=False, executable=False):
    return [{'marker':m,'severity':classify_local_console_binding_marker(m,path,generated,executable),'path':str(path)} for m in MARKERS if m in text]
def assert_no_xttrader_import(paths=None):
    hits=[]
    for base in paths or ['qmt_ai_trading/local_console','scripts/run_local_console_binding_review.py']:
    ...
    return hits
def assert_no_forbidden_binding_routes(routes): return [r for r in routes if classify_local_console_binding_route(getattr(r,'path',r)).get('forbidden')]
def assert_no_forbidden_js_actions(js): return [m for m in scan_local_console_binding_asset_for_forbidden_markers(js,'app.js',executable=True) if m['severity']=='CRITICAL']
