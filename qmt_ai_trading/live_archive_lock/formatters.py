...
def format_live_archive_lock_report_markdown(report:LiveArchiveLockReport)->str:
    lines=['# Stage51 Final Read-only Seal Review and Archive Lock','','## Decision',str(enum_value(report.decision)),'','## Safety Note',report.safety_note,'','## Evidence Summary',f"- total_evidence: {report.summary.get('total_evidence',len(report.evidence))}",f"- critical: {report.summary.get('critical',0)}",f"- warnings: {report.summary.get('warnings',len(report.warnings))}",'- read_only: True','- dry_run_only: True','- no_trade_authorization: True','- live_trading_enabled: False','- no_task_registered: True']
