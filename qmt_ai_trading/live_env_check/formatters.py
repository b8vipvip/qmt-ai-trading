...
def format_live_env_check_report_markdown(report: LiveEnvCheckReport) -> str:
    ...
    lines += ["", "## Safety note", report.safety_note, "", "## Next separate stage", "Stage 39: final manual authorization package for tiny-capital gray mode (still no execution). READY_FOR_ENV_REVIEW does not enable live trading or authorize orders."]
    return "\n".join(lines)+"\n"
