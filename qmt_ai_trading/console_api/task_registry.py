...
('research_score_etf','ETF 研究评分','RESEARCH','输出 ETF score/rank/reasons'),('model_lab_evaluate','模型实验评估','RESEARCH','dry-run 模型指标摘要'),('factor_scan','多因子扫描','RESEARCH','生成多因子候选'),('factor_research_dry_run','因子研究 dry-run','RESEARCH','生成因子研究上下文与报告'),
...
('agent_research_dry_run','Agent 投研 dry-run','AGENTS','运行多 Agent 投研工作流'),('agent_research_brief','Agent 投研简报','AGENTS','结构化建议'),('agent_risk_review','Agent 风险复核','AGENTS','结构化风险建议'),('agent_portfolio_review','Agent 组合复核','AGENTS','组合建议，不交易'),
('shadow_replay_backtest','Shadow Replay 回测','BACKTEST','回放式回测摘要'),('backtest_report','回测报告生成','BACKTEST','生成回测摘要'),('backtest_dashboard_dry_run','回测分析联动看板 dry-run','BACKTEST','生成回测分析与 Agent 对比看板'),
('monitoring_alert_dry_run','监控告警 dry-run','RISK','生成异常监控、告警与熔断 dry-run 看板'),
('market_replay_sandbox','Sandbox 行情回放','DATA','生成 Sandbox Market Data Gateway 与 Replay Bus 输出'),
('xtdata_boundary_dry_run','xtdata 边界适配器 dry-run','DATA','生成真实 xtdata 接入前边界检查输出'),
('xtdata_enable_dry_run','xtdata sandbox enable 确认 dry-run','DATA','生成 xtdata 启用前人工确认与配置审计输出'),
('xtdata_live_readonly_smoke','xtdata 只读行情 smoke','DATA','真实 xtdata 只读行情，默认 sandbox fallback'),
