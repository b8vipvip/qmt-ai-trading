...
GROUP_DEFS = {
    'all_a': {
        'name': 'QMT 全A股票池',
        'description': '从 QMT / xtdata 真实同步沪深北 A 股代码，用于构建全A可交易池的基础列表。',
        'aliases': ['沪深A股', 'A股', '上证A股', '深证A股', '北证A股'],
    },
    'etf': {
        'name': 'QMT ETF全量池',
        'description': '从 QMT / xtdata 真实同步场内 ETF / 基金代码，用于 ETF 轮动和主题池。',
        'aliases': ['沪深ETF', 'ETF', 'ETF基金', '沪深基金', '场内基金', '基金'],
    },
    'index': {
        'name': 'QMT 指数池',
        'description': '从 QMT / xtdata 真实同步指数代码，用于基准、行业和风格分析。',
        'aliases': ['沪深指数', '指数', '上证指数', '深证指数', '中证指数'],
    },
}
...
def _dedupe_symbols(values: Any) -> list[str]:
    ...
    seen: set[str] = set()
    for item in values:
        symbol = workbench_market._normalize_symbol(item)
...
def sync_qmt_universe(body: dict[str, Any] | None = None):
    ...
    if xtdata is None:
        result = {
            'status': 'FAILED',
            'syncedAt': _now(),
            'message': f'xtquant.xtdata 导入失败，无法同步真实列表：{import_status}',
            'importStatus': import_status,
            'clientProbe': client_probe,
            'groups': [],
            'groupCount': 0,
            'readOnly': True,
            'noXtTrader': True,
    ...
    result = {
        ...
        'readyGroupCount': ready_count,
        'totalSymbolCount': total_symbols,
        'source': 'xtdata.get_stock_list_in_sector',
        'readOnly': True,
        'noXtTrader': True,
        'noOrderSubmitted': True,
