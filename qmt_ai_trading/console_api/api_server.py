...
def _summary():
    runs=STORE.list()
    return payload(summary={'title':'QMT AI 统一本地量化控制台','mode':'dry-run/shadow/read-only','task_total':len(runs),'success_count':sum(r.status=='SUCCESS' for r in runs),'live_status':'DISABLED_FOR_SAFETY'})
...
def make_handler(static_dir=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        def _safe(self, fn):
            try:
                assert_http_method_allowed(self.command, urlparse(self.path).path); fn()
            except ConsoleSafetyError as e: _json(self,403,payload(ok=False,status='BLOCKED',error=str(e)))
            except Exception as e: _json(self,500,payload(ok=False,status='FAILED',error=str(e)))
        def do_GET(self): self._safe(self._get)
        def do_POST(self): self._safe(self._post)
        def do_PUT(self): self._safe(lambda: _json(self,405,payload(ok=False,status='METHOD_DISABLED')))
        def do_PATCH(self): self._safe(lambda: _json(self,405,payload(ok=False,status='METHOD_DISABLED')))
        def do_DELETE(self): self._safe(lambda: _json(self,405,payload(ok=False,status='METHOD_DISABLED')))
        def _get(self):
            p=urlparse(self.path).path
            if p=='/api/v1/health': return _json(self,200,payload(**{'service':'unified_local_console'},host='127.0.0.1'))
            if p in ROUTES: return _json(self,200,ROUTES[p]())
            if p=='/api/v1/console/summary': return _json(self,200,_summary())
            if p=='/api/v1/tasks/catalog': return _json(self,200,payload(tasks=[task_to_dict(t) for t in list_tasks()]))
            if p=='/api/v1/tasks': return _json(self,200,payload(tasks=[run_to_dict(r) for r in STORE.list()]))
            if p.startswith('/api/v1/tasks/'):
                parts=p.split('/'); rid=parts[4] if len(parts)>4 else ''; run=STORE.get(rid)
                if not run: return _json(self,404,payload(ok=False,status='DATA_MISSING',error='任务不存在'))
                if len(parts)>5 and parts[5]=='logs': return _json(self,200,payload(task_id=rid,logs=run.logs[-50:]))
                return _json(self,200,payload(task=run_to_dict(run)))
            return self._static(p)
        def _post(self):
            p=urlparse(self.path).path; raw=self.rfile.read(int(self.headers.get('Content-Length','0') or 0))
        ...
        def _static(self,p):
            if not root: return _json(self,404,payload(ok=False,status='DATA_MISSING',error='not found'))
            target=(root/('index.html' if p=='/' else p.lstrip('/'))).resolve()
            if not str(target).startswith(str(root)) or not target.exists() or target.is_dir(): return _json(self,404,payload(ok=False,status='DATA_MISSING',error='not found'))
