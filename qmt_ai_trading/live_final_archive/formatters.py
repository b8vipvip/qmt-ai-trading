...
def format_live_final_archive_report_markdown(report:LiveFinalArchiveReport)->str:
    lines=["# Stage50 Final Archive Review and Material Consistency Seal","","## Decision",str(enum_value(report.decision)),"","## Safety Note",report.safety_note,"","## Evidence Summary",f"- total_evidence: {report.summary.get('total_evidence',len(report.evidence))}",f"- critical: {report.summary.get('critical',0)}",f"- warnings: {report.summary.get('warnings',len(report.warnings))}","- read_only: True","- dry_run_only: True","- no_trade_authorization: True","- live_trading_enabled: False","- no_task_registered: True"]
