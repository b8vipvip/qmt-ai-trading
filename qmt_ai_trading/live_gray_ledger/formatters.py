...
SAFETY_NOTE_HEADING = "## Safety Note"
HUMAN_REVIEW_CHECKLIST_HEADING = "## Human Review Checklist"
NEXT_STAGE_PREVIEW_HEADING = "## Next Stage Preview"
HUMAN_REVIEW_CHECKLIST_ITEMS = [
    "- [ ] 确认 READY_FOR_MANUAL_REVIEW 只表示材料可供人工复核，不是实盘授权。",
    "- [ ] 确认本阶段不批准、不下单、不注册真实任务、不发送真实通知。",
    "- [ ] 确认 Risk Gate 与 Human Approval 仍是未来阶段必须边界。",
]
NEXT_STAGE_PREVIEW_TEXT = "Stage42 仍然只能做更严格的人工复核/只读模拟，不得直接实盘。"
...
def format_live_gray_ledger_report_markdown(report: LiveGrayLedgerReport) -> str:
    ...
    lines=["# Stage41 Live Gray Read-only Ledger","", "## Decision", str(enum_value(report.decision)), "", SAFETY_NOTE_HEADING, report.safety_note, "", "## Evidence Summary", json.dumps(report.summary, ensure_ascii=False), "", "| Severity | Status | Category | Path | Title | Summary |", "| --- | --- | --- | --- | --- | --- |"]
    ...
    lines += ["", HUMAN_REVIEW_CHECKLIST_HEADING, *HUMAN_REVIEW_CHECKLIST_ITEMS, "", "## Blocking Reasons"]
    lines += [f"- {x}" for x in report.blocked_reasons] or ["- None"]
    lines += ["", "## Warnings"] + ([f"- {x}" for x in report.warnings] or ["- None"])
    lines += ["", NEXT_STAGE_PREVIEW_HEADING, NEXT_STAGE_PREVIEW_TEXT, ""]
    return "\n".join(lines)
