...
def main():
    ...
    p.add_argument('--session-id', default=None); p.add_argument('--enable-account-readonly',action='store_true'); p.add_argument('--allow-import-xttrader',action='store_true'); p.add_argument('--allow-connect-trade-session',action='store_true'); p.add_argument('--allow-account-query',action='store_true'); p.add_argument('--allow-position-query',action='store_true'); p.add_argument('--manual-confirmed',action='store_true'); p.add_argument('--dry-run',action='store_true'); p.add_argument('--read-only',action='store_true')
    a=p.parse_args();
    if a.session_id:
        import os; os.environ['QMT_SESSION_ID_BASE']=str(a.session_id)
    print(run_account_readonly_stage91(a.repo_root,a.output_dir,a.enable_account_readonly,a.allow_import_xttrader,a.allow_connect_trade_session,a.allow_account_query,a.allow_position_query,a.manual_confirmed,True,True))
