...
$LogPath = Join-Path $LogDir "stage65_validation_$Stamp.log"
...
Write-Log "Stage65 validation started $($StartedAt.ToString('o'))"
Run-Command "py -m compileall -q qmt_ai_trading"
$tests = @("tests/test_final_authorization_stage39.py","tests/test_redline_review_stage40.py","tests/test_live_gray_ledger_stage41.py","tests/test_live_gray_review_stage42.py","tests/test_live_signature_freeze_stage43.py","tests/test_live_env_snapshot_stage44.py","tests/test_live_runbook_stage45.py","tests/test_live_signoff_stage46.py","tests/test_live_final_review_stage47.py","tests/test_live_archive_stage48.py","tests/test_live_consistency_stage49.py","tests/test_live_final_archive_stage50.py","tests/test_live_archive_lock_stage51.py","tests/test_live_lock_consistency_stage52.py","tests/test_live_archive_verification_stage53.py","tests/test_live_gap_clearance_stage54.py","tests/test_qmt_dryrun_calibration_stage55.py","tests/test_real_cache_quality_stage56.py","tests/test_live_gray_candidate_stage57.py","tests/test_live_gray_final_approval_stage58.py","tests/test_live_gray_readonly_seal_stage59.py","tests/test_pre_gray_final_review_stage60.py","tests/test_api_gateway_stage61.py","tests/test_local_console_stage62.py","tests/test_local_console_detail_stage63.py","tests/test_local_console_dashboard_stage64.py","tests/test_local_console_shell_stage65.py")
foreach ($t in $tests) { Run-Command "py -m pytest $t" }
Run-Command "py scripts\run_local_console_shell_review.py --repo-root . --output-dir local_console_shell_stage65 --local-console-dashboard-dir local_console_dashboard_stage64 --local-console-detail-dir local_console_detail_stage63 --local-console-dir local_console_stage62 --api-gateway-dir api_gateway_stage61 --output local_console_shell_stage65\local_console_shell_report.md --json-output local_console_shell_stage65\local_console_shell_report.json --manifest-output local_console_shell_stage65\shell_manifest.md --manifest-json-output local_console_shell_stage65\shell_manifest.json --route-output local_console_shell_stage65\route_map.md --route-json-output local_console_shell_stage65\route_map.json --asset-output local_console_shell_stage65\asset_index.md --asset-json-output local_console_shell_stage65\asset_index.json --binding-output local_console_shell_stage65\data_binding_placeholders.md --binding-json-output local_console_shell_stage65\data_binding_placeholders.json --safety-output local_console_shell_stage65\static_safety_boundary.md --safety-json-output local_console_shell_stage65\static_safety_boundary.json --plan-output local_console_shell_stage65\next_console_data_binding_plan.md --plan-json-output local_console_shell_stage65\next_console_data_binding_plan.json"
Print-File "local_console_shell_stage65\local_console_shell_report.md"
Print-File "local_console_shell_stage65\shell_manifest.md"
Print-File "local_console_shell_stage65\route_map.md"
Print-File "local_console_shell_stage65\asset_index.md"
Print-File "local_console_shell_stage65\data_binding_placeholders.md"
Print-File "local_console_shell_stage65\static_safety_boundary.md"
Print-File "local_console_shell_stage65\next_console_data_binding_plan.md"
Run-Command "py scripts\check_stage53_roadmap_plan.py"
Check-NoBackup "scripts" "validate_stage64.ps1.bak_stage64fix_*" "Stage64 validate"
Check-NoBackup "scripts" "validate_stage65.ps1.bak_stage65fix_*" "Stage65 validate"
Run-Command "py scripts\run_daily_pipeline.py --cache-root market_data_test_stage65 --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-shell-review --local-console-shell-review-output-dir local_console_shell_stage65"
Run-Command "py scripts\run_scheduled_daily_pipeline.py --warmup-universe --warmup-provider mock --universe-lookback-days 90 --warmup-frequency 1d --cache-root market_data_test_stage65 --data-source-mode cached_real_first --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-shell-review --local-console-shell-review-output-dir local_console_shell_stage65"
Run-Command "py scripts\register_daily_pipeline_task.py --warmup-universe --warmup-provider mock --universe-lookback-days 90 --warmup-frequency 1d --cache-root market_data --data-source-mode cached_real_first --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-shell-review --local-console-shell-review-output-dir local_console_shell --time 15:30"
Clean-PythonCache
...
if ($script:HadError) { Write-Log "Stage65 validation FAILED"; exit 1 }
Write-Log "Stage65 validation PASSED"
exit 0
