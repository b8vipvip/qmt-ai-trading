...
$LogPath = Join-Path $LogDir "stage63_validation_$Stamp.log"
...
function Clean-PythonCache {
    Write-Log ""
    Write-Log ">>> clean python cache artifacts"
    ...
    Write-Log "Python cache artifacts cleaned."
    Write-Log "EXIT_CODE=0"
}
Write-Log "Stage63 validation started $($StartedAt.ToString('o'))"
Run-Command "py -m compileall -q qmt_ai_trading"
$tests = @("tests/test_final_authorization_stage39.py","tests/test_redline_review_stage40.py","tests/test_live_gray_ledger_stage41.py","tests/test_live_gray_review_stage42.py","tests/test_live_signature_freeze_stage43.py","tests/test_live_env_snapshot_stage44.py","tests/test_live_runbook_stage45.py","tests/test_live_signoff_stage46.py","tests/test_live_final_review_stage47.py","tests/test_live_archive_stage48.py","tests/test_live_consistency_stage49.py","tests/test_live_final_archive_stage50.py","tests/test_live_archive_lock_stage51.py","tests/test_live_lock_consistency_stage52.py","tests/test_live_archive_verification_stage53.py","tests/test_live_gap_clearance_stage54.py","tests/test_qmt_dryrun_calibration_stage55.py","tests/test_real_cache_quality_stage56.py","tests/test_live_gray_candidate_stage57.py","tests/test_live_gray_final_approval_stage58.py","tests/test_live_gray_readonly_seal_stage59.py","tests/test_pre_gray_final_review_stage60.py","tests/test_api_gateway_stage61.py","tests/test_local_console_stage62.py","tests/test_local_console_detail_stage63.py")
foreach ($t in $tests) { Run-Command "py -m pytest $t" }
Run-Command "py scripts/run_local_console_detail_review.py --repo-root . --output-dir local_console_detail_stage63 --output local_console_detail_stage63/local_console_detail_report.md --json-output local_console_detail_stage63/local_console_detail_report.json --filter-output local_console_detail_stage63/filter_index.md --filter-json-output local_console_detail_stage63/filter_index.json --warnings-output local_console_detail_stage63/warnings.md --warnings-json-output local_console_detail_stage63/warnings.json --blocking-output local_console_detail_stage63/blocking_reasons.md --blocking-json-output local_console_detail_stage63/blocking_reasons.json --manifest-output local_console_detail_stage63/manifest_detail.md --manifest-json-output local_console_detail_stage63/manifest_detail.json --validation-output local_console_detail_stage63/validation_detail.md --validation-json-output local_console_detail_stage63/validation_detail.json --plan-output local_console_detail_stage63/next_console_overview_plan.md --plan-json-output local_console_detail_stage63/next_console_overview_plan.json"
Print-File "local_console_detail_stage63/local_console_detail_report.md"
Print-File "local_console_detail_stage63/filter_index.md"
Print-File "local_console_detail_stage63/warnings.md"
Print-File "local_console_detail_stage63/blocking_reasons.md"
Print-File "local_console_detail_stage63/manifest_detail.md"
Print-File "local_console_detail_stage63/validation_detail.md"
Print-File "local_console_detail_stage63/next_console_overview_plan.md"
...
Check-NoBackup "scripts" "validate_stage63.ps1.bak_stage63fix_*" "Stage63 validate"
Run-Command "py scripts/run_daily_pipeline.py --cache-root market_data_test_stage63 --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-detail-review --local-console-detail-review-output-dir local_console_detail_stage63"
Run-Command "py scripts/run_scheduled_daily_pipeline.py --warmup-universe --warmup-provider mock --universe-lookback-days 90 --warmup-frequency 1d --cache-root market_data_test_stage63 --data-source-mode cached_real_first --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-detail-review --local-console-detail-review-output-dir local_console_detail_stage63"
Run-Command "py scripts/register_daily_pipeline_task.py --warmup-universe --warmup-provider mock --universe-lookback-days 90 --warmup-frequency 1d --cache-root market_data --data-source-mode cached_real_first --research-start 2026-03-21 --research-end 2026-06-18 --research-frequency 1d --min-bars 40 --cached-strategy-top-n 2 --enable-local-console-detail-review --local-console-detail-review-output-dir local_console_detail --time 15:30"
Clean-PythonCache
...
if ($script:HadError) { Write-Log "Stage63 validation FAILED"; exit 1 }
Write-Log "Stage63 validation PASSED"
exit 0
