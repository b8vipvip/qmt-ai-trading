...
function Print-File { param([string]$Path) Write-Log "=== PRINT $Path ==="; if(Test-Path $Path){ Get-Content -LiteralPath $Path | Tee-Object -FilePath $LogPath -Append } else { Write-Log "MISSING: $Path"; $script:Failures += 1 } }
