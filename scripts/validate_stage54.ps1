...
$StartedAt = Get-Date; $Stamp = $StartedAt.ToString("yyyyMMdd_HHmmss"); $LogPath = Join-Path $LogDir "stage54_validation_$Stamp.log"; $script:HadError = $false
function Write-Log { param([string]$Message) $Message | Tee-Object -FilePath $LogPath -Append }
function Run-Command { param([string]$Command,[switch]$AllowFailure)
  Write-Log ""; Write-Log ">>> $Command"; $output=$null; $exitCode=0
  try { $output=& powershell -NoProfile -ExecutionPolicy Bypass -Command $Command 2>&1; $exitCode=$LASTEXITCODE; if ($null -eq $exitCode) { if ($?) { $exitCode=0 } else { $exitCode=1 } } } catch { $output=$_ | Out-String; $exitCode=1 }
  if ($null -ne $output) { foreach ($line in $output) { Write-Log ([string]$line) } }
  Write-Log ""; Write-Log "EXIT_CODE=$exitCode"; if (($exitCode -ne 0) -and (-not $AllowFailure)) { $script:HadError=$true }; return $exitCode
}
function Print-File { param([string]$Path) Run-Command "if (Test-Path -LiteralPath '$Path') { Get-Content -LiteralPath '$Path' -Encoding UTF8 } else { Write-Error 'SKIPPED: report file not generated ($Path)'; exit 1 }" }
Write-Log "Stage54 validation started $($StartedAt.ToString('o'))"
$tests=@('tests/test_final_authorization_stage39.py','tests/test_redline_review_stage40.py','tests/test_live_gray_ledger_stage41.py','tests/test_live_gray_review_stage42.py','tests/test_live_signature_freeze_stage43.py','tests/test_live_env_snapshot_stage44.py','tests/test_live_runbook_stage45.py','tests/test_live_signoff_stage46.py','tests/test_live_final_review_stage47.py','tests/test_live_archive_stage48.py','tests/test_live_consistency_stage49.py','tests/test_live_final_archive_stage50.py','tests/test_live_archive_lock_stage51.py','tests/test_live_lock_consistency_stage52.py','tests/test_live_archive_verification_stage53.py','tests/test_live_gap_clearance_stage54.py')
Run-Command "py -m compileall -q qmt_ai_trading"
foreach ($t in $tests) { Run-Command "py -m pytest $t" }
$rc=Run-Command "py scripts\run_live_gap_clearance.py --repo-root . --output-dir live_gap_clearance_stage54 --output live_gap_clearance_stage54\live_gap_clearance.md --json-output live_gap_clearance_stage54\live_gap_clearance.json --remediation-output live_gap_clearance_stage54\evidence_remediation.md --remediation-json-output live_gap_clearance_stage54\evidence_remediation.json --human-output live_gap_clearance_stage54\human_closure_recheck.md --human-json-output live_gap_clearance_stage54\human_closure_recheck.json --plan-output live_gap_clearance_stage54\next_readonly_check_plan.md --plan-json-output live_gap_clearance_stage54\next_readonly_check_plan.json" -AllowFailure
Print-File "live_gap_clearance_stage54\live_gap_clearance.md"; Print-File "live_gap_clearance_stage54\evidence_remediation.md"; Print-File "live_gap_clearance_stage54\human_closure_recheck.md"; Print-File "live_gap_clearance_stage54\next_readonly_check_plan.md"
if ($rc -ne 0) { $script:HadError=$true }
Run-Command "py scripts\check_stage53_roadmap_plan.py"
Run-Command "powershell -NoProfile -Command \"if (Get-ChildItem -Path scripts -Filter 'validate_stage53.ps1.bak_*' -ErrorAction SilentlyContinue) { Write-Error 'Stage53 backup file still exists'; exit 1 } else { Write-Output 'Stage53 backup files cleaned.' }\""
...
$FinishedAt=Get-Date
if ($script:HadError) { Write-Log "Stage54 validation finished $($FinishedAt.ToString('o')) EXIT_CODE=1"; Write-Error "Stage54 validation failed. Log: $LogPath"; exit 1 }
Write-Log "Stage54 validation finished $($FinishedAt.ToString('o')) EXIT_CODE=0"; exit 0
