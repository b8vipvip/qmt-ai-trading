...
function Print-File {
    ...
    Write-Log ""
    Write-Log ">>> if (Test-Path -LiteralPath '$Path') { Get-Content -LiteralPath '$Path' -Encoding UTF8 } else { Write-Output 'SKIPPED: report file not generated ($Path)' }"
...
Write-Log "Stage53 validation started $($StartedAt.ToString('o'))"
...
Run-Command "py scripts\run_live_archive_verification.py --repo-root . --output-dir live_archive_verification_stage53 --output live_archive_verification_stage53\live_archive_verification.md --json-output live_archive_verification_stage53\live_archive_verification.json --locked-output live_archive_verification_stage53\locked_material_review.md --locked-json-output live_archive_verification_stage53\locked_material_review.json --human-output live_archive_verification_stage53\human_closure_recheck.md --human-json-output live_archive_verification_stage53\human_closure_recheck.json --plan-output live_archive_verification_stage53\next_readonly_check_plan.md --plan-json-output live_archive_verification_stage53\next_readonly_check_plan.json"

Print-File "live_archive_verification_stage53\live_archive_verification.md"
Print-File "live_archive_verification_stage53\locked_material_review.md"
Print-File "live_archive_verification_stage53\human_closure_recheck.md"
Print-File "live_archive_verification_stage53\next_readonly_check_plan.md"

Run-Command "py scripts\check_stage53_roadmap_plan.py"

Run-Command "py scripts\warmup_etf_universe.py --lookback-days 40 --frequency 1d --provider mock --cache-root market_data_test_stage53"

Run-Command "py scripts\run_daily_pipeline.py --cache-root market_data_test_stage53 --research-start 2026-05-09 --research-end 2026-06-18 --research-frequency 1d --min-bars 20 --cached-strategy-top-n 2 --enable-live-archive-verification --live-archive-verification-output-dir live_archive_verification_stage53"
...
if ($script:HadError) {
    Write-Log "Stage53 validation finished $($FinishedAt.ToString('o')) EXIT_CODE=1"
    Write-Error "Stage53 validation failed. Log: $LogPath"
    exit 1
}

Write-Log "Stage53 validation finished $($FinishedAt.ToString('o')) EXIT_CODE=0"
exit 0
