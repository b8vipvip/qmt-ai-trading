...
### 阶段八十：因子候选池到 Strategy Engine dry-run TradeIntent 联调层 + 总控脚本日志落盘真实修复

- 当前状态：已完成。
- 将 Stage79 `factor_candidates` 接入 Strategy Engine，生成 `source=factor_strategy_stage80`、`dry_run=True` 的 TradeIntent，并通过 Risk Gate dry-run 输出拒绝/阻断原因。
- 真实修复父级 `run_qmt_tasks.ps1` 验收日志落盘，日志路径为 `validation_logs/stage*_validation_*.log`，Stage80 验收会自动执行安装脚本。
- 仍不接实盘、不下单、不查询真实账户、不调用 xttrader、不自动 approve。

### 阶段八十一预告：TradingAgents 多 Agent 投研工作台与模型用途映射联调层

Stage81 将 Stage78 的 AI 模型配置中心接入 TradingAgents，让技术面、基本面、情绪新闻、风险和组合经理 Agent 选择不同模型运行投研任务；仍然不是实盘，不真实下单。
