...
| 验证脚本 | `scripts/validate_local_console_workbench.ps1` | 校验前端、API、危险旗标和关键接口。 |
...
前端、API、日常操作中心、Portfolio 预览和安全旗标都正常后，再考虑提交删除或保留归档。`artifacts\cleanup_archive\` 是本地备份，不要提交到 GitHub。
...
3. **接入真实 Data Hub 外部源**：AkShare / Tushare / BaoStock、指数成分、ETF、行业、财务、新闻。
4. **接入研究层模型**：Alpha158 / Alpha360 / Alpha101、LightGBM / Lasso / MLP。
5. **Agent 投研结构化输出**：技术面、基本面、新闻情绪、风险、组合经理 Agent。
6. **实盘前安全审计**：在任何小资金实盘前先做 go/no-go 审计和人工二次确认。
