...
def collect_shadow_replay_batch():
    ...
    display = {
        ...
        "non_overlapping_summary": summary.get("non_overlapping_summary"),
        "rolling_summary": summary.get("rolling_summary"),
        "full_period_summary": summary.get("full_period_summary"),
        "overfitting_score": summary.get("overfitting_score"),
        "underfitting_score": summary.get("underfitting_score"),
        "regime_fit_score": summary.get("regime_fit_score"),
        "robustness_score": summary.get("robustness_score"),
        "overfit_warning": summary.get("overfit_warning"),
        "underfit_warning": summary.get("underfit_warning"),
        "risk_warning": summary.get("risk_warning"),
        "optimization_recommendation": summary.get("optimization_recommendation"),
...
def _render_shadow_replay_batch_markdown(value):
    ...
    lines = [
        ...
        "- 鲁棒性评分：{0}".format(display.get("robustness_score")),
        "- 市场环境适配评分：{0}".format(display.get("regime_fit_score")),
        "- 年度非重叠摘要：{0}".format(json.dumps(display.get("non_overlapping_summary") or {}, ensure_ascii=False)),
        "- 滚动区间摘要：{0}".format(json.dumps(display.get("rolling_summary") or {}, ensure_ascii=False)),
        "- 长周期摘要：{0}".format(json.dumps(display.get("full_period_summary") or {}, ensure_ascii=False)),
        "- 过拟合评分：{0}".format(display.get("overfitting_score")),
        "- 欠拟合评分：{0}".format(display.get("underfitting_score")),
        "- 过拟合警告：{0}".format(display.get("overfit_warning")),
        "- 欠拟合警告：{0}".format(display.get("underfit_warning")),
        "- 风控警告：{0}".format(display.get("risk_warning")),
        "- 优化建议：{0}".format(display.get("optimization_recommendation")),
